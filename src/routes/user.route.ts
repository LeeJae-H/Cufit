...
router.post("/", updateUserProfile); // idtoken 받아서 사용자 정보 수정
router.delete("/", deleteUser); // idtoken 받아서 사용자 정보 삭제
...
router.post("/product/buy", buyProduct); // product 구매
router.post("/product/use", useProduct); // 제품 사용
router.post("/product/review", reviewProduct); // product review 쓰기

router.get("/transaction-credit", getCreditTransaction); // idtoken 받아서 credit transaction 조회 
router.get("/reward-advertisement", getAdrewardAmount); // 광고 시청으로 얻을 수 있는 credit 양
router.post("/credit-advertisement", getAdreward); // 광고 시청으로 얻은 credit
