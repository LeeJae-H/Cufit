...
router.get('/product', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (type === "Filter") {
        try {
            const filtered = yield filter_1.Filter.aggregate([
                {
                    $lookup: {
                        from: "auth",
                        localField: "_id",
            ...
            const result = yield filter_1.Filter.find({ _id: filteredIds })
                .populate('likedCount')
        ...
        try {
            const filtered = yield guideline_1.Guideline.aggregate([
                {
                    $lookup: {
                        from: "auth",
                        localField: "_id",
            ...
            const filteredIds = filtered.map(item => item._id).reverse();
            const result = yield guideline_1.Guideline.find({ _id: filteredIds })
                .populate('likedCount')
                .populate('wishedCount')
                .populate('usedCount')
                .populate('authStatus')
